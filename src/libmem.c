...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, addr_t size, addr_t *alloc_addr)
{
  ...
  if (caller->krnl->running_list == NULL) {
      printf("[DEBUG] __alloc: No running list in kernel\n");
...
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller)
{
  ...
  /* [UPDATED] GỌI SYSCALL 17 (MEMIO READ) - Bảo mật */
  // Không được gọi trực tiếp MEMPHY_read từ User Space
  struct sc_regs regs;
  regs.a1 = SYSMEM_IO_READ;
  regs.a2 = phyaddr;
  regs.a3 = 0; // Buffer nhận dữ liệu

  syscall(caller->krnl, caller->pid, 17, &regs);

  *data = (BYTE)regs.a3; // Lấy dữ liệu trả về
